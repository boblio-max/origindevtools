...
def cli():
    ...
    while running:
        try:
            ...
            elif cmd_or_file == "venv":
                if len(parts) < 3:
                    print("Error: Please specify a name for the virtual environment.")
                else:
                    venv_name = parts[2]
                    create_venv(venv_name, get_working_directory())

            elif cmd_or_file == "activate":
                if len(parts) < 3:
                    print("Error: Please specify a name for the virtual environment to activate.")
                else:
                    venv_name = parts[2]
                    init_venv(venv_name, get_working_directory())
